...
class valac(Task.Task):
    ...
    def run(self):
        ...
        if self.is_lib:
            ...
            cmd.append('--includedir=' + self.name)
            cmd.append('--library=' + self.target)
            cmd.append('--header=%s.h' % (self.target))
            if self.gir:
...
def vala_file(self, node):
    ...
    if not valatask:
        ...
        if valatask.is_lib:
            valatask.outputs.append(self.path.find_or_declare('%s/%s.h' %
                                                     (self.name, self.target)))
            valatask.outputs.append(self.path.find_or_declare('%s.vapi' %
                                                              self.target))
...
@conf
def check_vala(self, min_version=(0,18,0), branches=('0.22', '0.20', '0.18')):
    """
